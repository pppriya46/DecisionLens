...
from api.search_service import get_db_connection, search_duplicate_incidents
...
def fetch_query_records(ticket_ids):
...
def evaluate_pair(row, query_record, top_k, top_n):
    ...
    returned_ticket_ids = [item["ticket_id"] for item in result["results"]]
...
def main():
    ...
    query_records = fetch_query_records(query_ticket_ids)

    missing_queries = [ticket_id for ticket_id in query_ticket_ids if ticket_id not in query_records]
    if missing_queries:
        raise SystemExit(f"Missing query tickets in database: {', '.join(missing_queries[:10])}")

    evaluations = [
        evaluate_pair(row, query_records[row["query_ticket_id"]], top_k=args.top_k, top_n=args.top_n)
        for row in rows
    ]
    ...
    for label, stats in summary["by_label"].items():
        ...
        print(f"- {label}: {stats['hits']}/{stats['total']} ({stats['hit_rate']:.2%}), avg rank={avg_rank}")
