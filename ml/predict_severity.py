...
def predict_severity(
    ...
    Returns a dict with:
    - predicted_priority: the predicted class
    ...
    def safe_encode(encoder, value):
        """Encode a value, returning 0 if unseen."""
        classes = list(encoder.classes_)
        return classes.index(value) if value in classes else 0

    category_encoded     = safe_encode(encoders['category'],     str(category).strip())
    subcategory_encoded  = safe_encode(encoders['subcategory'],  str(subcategory).strip())
    contact_type_encoded = safe_encode(encoders['contact_type'], str(contact_type).strip())

    # Only the 12 features the model was trained with
...
if __name__ == "__main__":
    ...
    print(f"  All probs : {result1['all_probabilities']}\n")
    ...
    print(f"  All probs : {result2['all_probabilities']}\n")
    ...
    result3 = predict_severity(
        ...
        made_sla=True,
        knowledge=False,
        initial_message="My payment is not going through at checkout"
    )
    print("Test 3 — Billing bug:")
    print(f"  Predicted : {result3['predicted_priority']}")
    print(f"  Confidence: {result3['confidence']:.1%}")
    print(f"  All probs : {result3['all_probabilities']}\n")